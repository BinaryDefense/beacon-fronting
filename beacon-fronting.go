...
func main() {
	...
	frontSNIPtr := flag.Bool("frontsni", false, "include -frontsni to use the fronted domain as the TLS SNI field")
	blankSNIPtr := flag.Bool("blanksni", false, "include -blanksni to use a blank string as the TLS SNI field")
	...
	if *frontPtr {
		servername := *backendDomainPtr
		if *frontSNIPtr {
			servername = *frontedDomainPtr
		}
		if *blankSNIPtr {
			servername = ""
		}
